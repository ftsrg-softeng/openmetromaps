...
public class Node {
   ...
   int findEntry(float minX, float minY, float maxX, float maxY, int id) {
      for (int i = 0; i < this.entryCount; i++) {
         if (id == this.ids[i] && this.entriesMinX[i] == minX && this.entriesMinY[i] == minY && this.entriesMaxX[i] == maxX && this.entriesMaxY[i] == maxY) {
            return i;
         }
      }
   ...
   public int getId(int index) {
      return index < this.entryCount ? this.ids[index] : -1;
   }
   ...
   public Rectangle getEntryMbb(int index) {
      return new Rectangle(this.entriesMinX[index], this.entriesMinY[index], this.entriesMaxX[index], this.entriesMaxY[index]);
   }
}
